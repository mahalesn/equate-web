...
export function useEquateGame() {
  ...
  const [rightRemain, setRightRemain] = useState<Record<string, number>>(() =>
    countDigits(rightSet)
  )
  useEffect(() => {
    setLeftRemain(countDigits(leftSet))
    setRightRemain(countDigits(rightSet))
    setLeftExpr('')
    setRightExpr('')
    setActive('left')
    setWinTick(false)
  }, [leftSet.toString(), rightSet.toString()])
  function countDigits(arr: string[]) {
  ...
  const getExpr = useCallback(
    () => (active === 'left' ? leftExpr : rightExpr),
    [active, leftExpr, rightExpr]
  )
  const setExpr = useCallback(
    (v: string) => (active === 'left' ? setLeftExpr : setRightExpr)(v),
    [active]
  )
  const isDigitAllowed = useCallback(
    (d: string) => {
      const pool = active === 'left' ? leftRemain : rightRemain
      return (pool[d] ?? 0) > 0
    },
    [active, leftRemain, rightRemain]
  )
  ...
  const onBack = useCallback(() => {
    ...
    if (isDigit(removed)) {
      if (active === 'left') {
        setLeftRemain((p) => ({ ...p, [removed]: (p[removed] ?? 0) + 1 }))
      } else {
        setRightRemain((p) => ({ ...p, [removed]: (p[removed] ?? 0) + 1 }))
      }
    }
  }, [active, getExpr, setExpr])
  const onClear = useCallback(() => {
    ...
    for (const ch of s) {
      if (isDigit(ch)) {
        if (active === 'left') {
          setLeftRemain((p) => ({ ...p, [ch]: (p[ch] ?? 0) + 1 }))
        } else {
          setRightRemain((p) => ({ ...p, [ch]: (p[ch] ?? 0) + 1 }))
        }
      }
    }
    setExpr('')
  ...
  const leftVal = useMemo(() => safeEval(leftExpr), [leftExpr])
  const rightVal = useMemo(() => safeEval(rightExpr), [rightExpr])
  const leftValueStr = leftVal ? leftVal.toString() : leftExpr ? '?' : ''
  ...
  useEffect(() => {
    if (win) {
      setWinTick(false)
      requestAnimationFrame(() => setWinTick(true))
    }
  }, [win])
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key >= '0' && e.key <= '9') {
        if (isDigitAllowed(e.key)) onDigit(e.key)
        e.preventDefault()
        return
      }
      if (['+', '-', '*', '/'].includes(e.key)) {
