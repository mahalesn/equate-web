export class Frac {
  ...
  constructor(n: bigint | number, d: bigint | number = 1){
    let nb = BigInt(n), db = BigInt(d)
    if(db === 0n) throw new Error('division by zero')
    if(db < 0n){ nb = -nb; db = -db }
  ...
  div(o: Frac){ if(o.n===0n) throw new Error('division by zero'); return new Frac(this.n*o.d, this.d*o.n) }
  toString(){ return this.d === 1n ? this.n.toString() : `${this.n}/${this.d}` }
}
function gcd(a: bigint, b: bigint){ while(b){ const t=a%b; a=b; b=t } return a }
